...
Let's look at the Rex sole data.
...
library(here)
load(here('code/Bio_All_NWFSC.Combo_2023-01-19.rda'))
rex.bio <- Data
```
...
Note even though the estimates of k did not really change, and there is still substantial, though less, correlation between $L_{a_2}$ and $k$, this is the parameterization stock synthesis uses.
...
.pull-right-narrow[
There is some fanning, especially between ages 0-10
]
...
Recall, if $\log(X)\sim N(\log(\mu), \sigma^2)$, then
$$E[X] = \mu + \tfrac{\sigma^2}{2}$$

To fit log normal errors to the *mean* length, we will need to write our own likelihood function.
